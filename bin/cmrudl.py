...
class Main():
    ...
    def assert_status_code(code, expected):
        if code != expected:
            raise Exception('Invalid status code: %s expected: %s' %
                            (code, expected))
    ...
    def request_download(self, url, dest, progress, cont=False):
        ...
        with open(dest, 'ab' if cont else 'wb') as fp:
            ...
            if continued and code == 416:
                progress(self.DL_PROGRESS_DONE, start,
                         time.time(), offset, 0, offset, offset)
                return

            self.assert_status_code(res.getcode(), status)

            headers = res.info()
            content_length = self.request_header_get(
                headers, 'content-length', int)

            total = None if content_length is None else (
            ...
            while True:
                ...
                if not added:
                    break
                size += added
                progress(self.DL_PROGRESS_READ, start,
                         time.time(), offset, added, size, total)
                fp.write(data)
                progress(self.DL_PROGRESS_WROTE, start,
                         time.time(), offset, added, size, total)

            progress(self.DL_PROGRESS_DONE, start,
                     time.time(), offset, 0, size, total)
    ...
    def fetch_storage(self, url):
        ...
        if weblink_get_len != 1:
            raise Exception(
                'Unexpected dispatcher.weblink_get count: %s' % (
                    weblink_get_len)
            )
